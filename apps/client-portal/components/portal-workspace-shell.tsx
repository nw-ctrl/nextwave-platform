                      ...
                      <SidebarMenuButton asChild isActive={active} tooltip={item.label} size="lg" className="rounded-xl px-4 hover:bg-primary/5 active:scale-[0.98] transition-all">
                        <Link href={item.href}>
                          ...
                          <div className="flex min-w-0 flex-col group-data-[collapsible=icon]:hidden">
                            <span className={`truncate text-sm ${active ? "font-semibold text-foreground" : "font-medium text-muted-foreground"}`}>{item.label}</span>
          ...
          <header className="sticky top-0 z-20 bg-background/90 backdrop-blur-3xl border-b border-border/50 px-4 py-4 md:px-8">
            <div className="mx-auto flex w-full max-w-7xl items-center gap-4">
              ...
              <div className="flex flex-1 items-center gap-3">
                <Button type="button" variant="outline" className="h-11 flex-1 justify-between rounded-2xl border-none bg-white/40 dark:bg-black/20 px-4 text-left text-muted-foreground shadow-sm transition-all hover:bg-white/60 sm:max-w-sm" onClick={() => setCommandOpen(true)}>
                  <span className="flex items-center gap-2 text-sm font-medium"><Search className="size-4 text-primary mb-0.5" />Search everything...</span>
                  <span className="hidden items-center gap-1 rounded-lg bg-black/5 dark:bg-white/5 px-2 py-1 text-[10px] font-bold uppercase tracking-widest opacity-40 sm:inline-flex"><Command className="size-2.5" />K</span>
                </Button>
                    ...
                    <DropdownMenuContent align="end" className="w-64 rounded-[24px] border-none glass p-2 mt-2">
                      <DropdownMenuLabel className="p-3">
                        <div className="flex flex-col gap-0.5">
                          ...
                          <span className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground opacity-60">{currentMembership.clinicName}</span>
                        </div>
                      </DropdownMenuLabel>
                      <DropdownMenuSeparator className="bg-muted-foreground/10 mx-2" />
                      <DropdownMenuItem asChild className="rounded-xl h-11 px-3"><Link href="/settings"><Settings2 className="mr-2 size-4 opacity-70" />Settings</Link></DropdownMenuItem>
                      <DropdownMenuItem onClick={handleOpenBillingSettings} className="rounded-xl h-11 px-3"><ArrowUpRight className="mr-2 size-4 opacity-70" />Billing</DropdownMenuItem>
                      <DropdownMenuSeparator className="bg-muted-foreground/10 mx-2" />
                      <div className="flex p-1 gap-1">
                        <Button variant="ghost" size="sm" className="flex-1 rounded-xl h-9 hover:bg-primary/5" onClick={() => setTheme("light")}><SunMedium className="size-4 mr-2" /> Light</Button>
                        <Button variant="ghost" size="sm" className="flex-1 rounded-xl h-9 hover:bg-primary/5" onClick={() => setTheme("dark")}><MoonStar className="size-4 mr-2" /> Dark</Button>
                      </div>
                      <DropdownMenuSeparator className="bg-muted-foreground/10 mx-2" />
                      <DropdownMenuItem onClick={handleLogout} className="rounded-xl h-11 px-3 text-destructive focus:text-destructive focus:bg-destructive/5"><LogOut className="mr-2 size-4" />Sign out</DropdownMenuItem>
                    </DropdownMenuContent>
                  </DropdownMenu>
                </div>
              </div>
