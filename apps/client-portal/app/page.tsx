  ...
  return (
    ...
    >
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.3fr)_minmax(320px,0.7fr)]">
        <Card className="overflow-hidden rounded-[36px] border border-slate-200 bg-white shadow-[0_28px_80px_rgba(15,23,42,0.07)]">
          <CardContent className="relative p-8 md:p-10">
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,_rgba(14,165,233,0.12),_transparent_28%),radial-gradient(circle_at_bottom_left,_rgba(15,23,42,0.05),_transparent_30%)]" />
                ...
                {quickActions.map((action) => {
                  ...
                  return (
                    <Link
                      key={action.href}
                      href={action.href}
                      className="group rounded-[28px] border border-slate-200 bg-white/80 p-5 shadow-sm transition-all hover:-translate-y-0.5 hover:border-sky-200 hover:shadow-[0_18px_40px_rgba(15,23,42,0.08)]"
                    >
        ...
        <Card className="overflow-hidden rounded-[36px] border border-slate-200 bg-slate-950 text-white shadow-[0_28px_80px_rgba(15,23,42,0.16)]">
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
        <Card className="rounded-[34px] border border-slate-200 bg-white shadow-[0_18px_48px_rgba(15,23,42,0.05)]">
            ...
            {summary.latestPatients.length === 0 ? (
              ...
              summary.latestPatients.map((patient) => (
                <Link
                  ...
                  className="group grid gap-4 rounded-[24px] border border-slate-200 bg-slate-50/70 px-5 py-4 transition-all hover:border-sky-200 hover:bg-white hover:shadow-sm md:grid-cols-[minmax(0,1fr)_auto]"
                >
                  ...
                  <div className="flex items-center gap-3 text-xs text-slate-500">
                    <span className="rounded-full border border-slate-200 bg-white px-3 py-1">Open chart</span>
            ...
            <CardContent className="space-y-3">
              {summary.latestVisits.length === 0 ? (
                ...
                summary.latestVisits.map((visit) => (
                  <div key={visit.id} className="rounded-[24px] border border-slate-200 bg-slate-50/70 px-5 py-4">
                    <div className="flex items-center justify-between gap-4">
          ...
          <Card className="rounded-[34px] border border-slate-200 bg-white shadow-[0_18px_48px_rgba(15,23,42,0.05)]">
            ...
            <CardContent className="grid gap-3">
              {["Search and open patients", "Register a new patient", "Add diagnosis and plan", "Print visit prescription view"].map((item) => (
                <div key={item} className="flex items-center gap-3 rounded-[22px] border border-slate-200 bg-slate-50/70 px-4 py-3 text-sm text-slate-700">
